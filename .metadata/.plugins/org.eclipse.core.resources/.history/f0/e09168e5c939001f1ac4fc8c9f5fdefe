...
public class App {
	...
	public static void main(String[] args) {
		...
		try {
			...
			conn.setAutoCommit(false);  // execute~ 메서드 실행 후 commit 수행 금지  -> 명시적 commit or rollback 필요
			...
			// 마지막 commit() 또는 rollback() 실행 후 수행된 모든 SQL 작업을 취소
			System.out.println("계좌 이체 실패");
			try { conn.rollback(); } catch (Exception ex2) {}  // 이전 변경 사항을 취소
		} finally {
			// 6. 연결 닫기
