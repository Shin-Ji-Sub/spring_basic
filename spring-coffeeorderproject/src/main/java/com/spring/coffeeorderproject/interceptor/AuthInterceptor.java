package com.spring.coffeeorderproject.interceptor;
...
public class AuthInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (uri.contains("/admin-home") || uri.contains("/orders-management") || uri.contains("/user-management") || uri.contains("/product-management") || uri.contains("/revenue-management")
			...
			|| uri.contains("/userPayment") || uri.contains("/userMyPage")
			|| uri.contains("reviewboardwrite") || uri.contains("inquiry1on1write") || uri.contains("announcementwrite")) {
			if (user == null) {
				
				response.sendRedirect("/spring-coffeeorderproject/userAccount/login");
				return false;
			}
		}
